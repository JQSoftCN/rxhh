...
func (c *appContext) Insert() {
	...
	//afed,_:=result.RowsAffected()

	//fmt.Println("insert ",strconv.Itoa(int(afed)))
...
func main() {
	c, err := connectDB("postgres", "user=hh password=admin dbname=hh sslmode=disable")
	if err != "" {
		print(err)
	}
